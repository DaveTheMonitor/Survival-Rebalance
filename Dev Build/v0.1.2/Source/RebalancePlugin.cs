...
namespace DaveTheMonitor
{
    public class RebalancePlugin : ITMPlugin
    {
        ...
        #region GUI
        string indevText = "Survival Rebalance Dev Build v0.1.2";
        ...
        Dictionary<ActorType, ActorTypeDataXML> npcTypeData = new Dictionary<ActorType, ActorTypeDataXML>();
        #endregion
        ...
        void ITMPlugin.InitializeGame(ITMGame game)
        {
            ...
            foreach (ActorTypeDataXML data in Globals1.NpcTypeData)
            {
                ...
                npcTypeData.Add(data.ActorType, data);
            }
            ...
            if (config.NewCombat)
            {
                foreach (ItemDataXML data in Globals1.ItemData)
                {
                    ...
                    if (data.ItemID == Item.AmuletOfFury)
                        data.Desc = $"An Enchanted Greenstone Amulet. Adds an extra {config.AmuletOfFuryCritChance}% chance of landing a critical hit on your opponent.";
                    i++;
                }
            }
        ...
        void UnlockLegendaryItems(Item item, ITMPlayer player)
        {
            ...
            game.RunSingleScriptCommand($"CaveIn [{(int)player.Position.X},{(int)player.Position.Y},{(int)player.Position.Z}]", player);
        }
        ...
        void SwingItem(Item item, ITMHand hand)
        {
            if (hand.Player != null)
            {
                ...
                else if ((item == Item.Hand || player.IsItemEquippedAndUsable(item)) && config.NewCombat)
                {
                    int itemID = (int)item;
                    ITMActor target = player.ActorInReticle;
                    if (target != null && newCombatData.ContainsKey(item))
                    {
                        ActorTypeDataXML actorData = npcTypeData[target.ActorType];
                        float critBoxOffset = actorData.CriticalBoxOffset;
                        Vector2 critBoxScale = actorData.CriticalBoxScale;
                        BoundingBox critBox = new BoundingBox(
                            new Vector3(target.Position.X - critBoxScale.X, target.Position.Y + (critBoxOffset * 2), target.Position.Z - critBoxScale.X),
                            new Vector3(target.Position.X + critBoxScale.X, target.Position.Y + (critBoxOffset * 2) + (critBoxScale.Y * 2), target.Position.Z + critBoxScale.X)
                            );
                        if (config.DebugMode)
                        {
                            StudioForge.TotalMiner.Graphics.ParticleData particle = new StudioForge.TotalMiner.Graphics.ParticleData()
                            {
                                Duration = 250,
                                StartColor = new Color(255, 0, 0, 50),
                                EndColor = new Color(255, 0, 0, 50),
                                Size = new Vector4(critBox.Max - critBox.Min, 1),
                            };
                            world.AddParticle(new Vector3(target.Position.X, critBox.Min.Y + (particle.Size.Y / 2), target.Position.Z), ref particle);
                        }
                        NewCombatData data = newCombatData[item];
                        bool hit = false;
                        bool crit = false;
                        Ray ray = new Ray() { Position = player.EyePosition, Direction = player.ViewDirection };
                        if (ray.Intersects(critBox) < data.Reach)
                        {
                            hit = true;
                            crit = player.IsItemEquippedAndUsable(Item.AmuletOfFury) ? rand.Next(0, 100) < config.CritChance + config.AmuletOfFuryCritChance : rand.Next(0, 100) < config.CritChance;
                        }
                        else if (ray.Intersects(target.Box) < data.Reach)
                        {
                            hit = true;
                            crit = false;
                        }
                        ...
                        if (hit)
                        {
                            if (crit)
                            {
                                CoreGlobals.Message.ShowMessage("Critical!", new Vector2(rand.Next(-config.CritMessageSpeed, config.CritMessageSpeed), 60), 2, 1.6f, config.CritMessageColor);
                                if (player.IsItemEquippedAndUsable(Item.AmuletOfFury))
                                {
                                    InventoryItem invItem = player.Inventory.Items[player.GetItemEquippedSlot(Item.AmuletOfFury)];
                                    invItem.Durability -= 1;
                                    player.Inventory.Items[player.GetItemEquippedSlot(Item.AmuletOfFury)] = invItem;
                                }
                            }
                            float damage = crit ? rand.Next((int)(data.Damage * 0.95f), (int)(data.Damage * 1.05f)) * 1.5f : rand.Next((int)(data.Damage * 0.9f), (int)(data.Damage * 1.1f));
                            int defence = npcLevelData[prevDifficulty].ContainsKey(actorData.LevelType) ? npcLevelData[prevDifficulty][actorData.LevelType].DefenceLevel : Globals1.NpcLevelData[(int)actorData.LevelType].DefenceLevel;
                            damage *= MathHelper.Clamp(damage * 0.7f / defence, 0.05f, 1);
                            Vector3 knockback = player.ViewDirection * (damage / target.MaxHealth);
